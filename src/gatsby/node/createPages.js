...
module.exports = async ({ graphql, actions, reporter }, options) => {
    ...
    // Default blog settings if not provided
    const blogSettings = options?.blogSettings || {
        path: '/blog',
    ...
    } else if (data.allArticle.articles.length !== 0 && (!blogSettings || !blogSettings.path)) {
        throw new Error(`No path for ArticleListing page in gatsby-config specified`);
    } else if (data.allArticle.articles.length === 0 && !blogSettings) {
        reporter.info('Blog disabled, skipping articles page creation');
        return;
    }

    // Create ArticleListing page
    const articleListingPageSlug = blogSettings.path.replace(/\/\/+/g, '/'); // remove duplicate slashes
