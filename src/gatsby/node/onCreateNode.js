...
module.exports = ({ node, getNode, actions, createNodeId, reporter }, options) => {
    ...
    if (node.internal.type === 'MarkdownRemark' && node.parent) {
        ...
        if (/^articles[\/\\]/.test(getNode(node.parent).relativeDirectory)) {
            ...
            if (!blogSettings || !blogSettings.path) {
                reporter.info('Blog settings not configured, skipping article processing');
                return;
            }

            const nodeFields = {
                slug: createSlugForFolder({ node: articleNode, getNode }, { blogSettings }),
